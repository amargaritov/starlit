...
//    printf("Started iteration %d, current state f = %d, read line %s, with len, %d\n", num_iteration, f, s, line_length);
...
//        printf("Bad string, type 3: %s\n", s);
      ...
      if (f == 3) { // expect contributor
        if (*(int *)&s[6] == 'oc/<') {
//          printf("Processing f = 3, put %s to t2\n", (char *)&s[7]);
          fputs(&s[7], t2);
...
//          printf("Processing f = 3, put %s to t2\n", (char *)&s[9]);
 ...
 //         printf("Processing f != 3, put %s to t2\n", (char *)&s[5]);
  ...
  //        printf("Processing f != 3, put %s to t2\n", (char *)&s[7]);
        ...
        if (*(int *)&s[6] == 'noc<') {
   //       printf("Assign f = 3%s\n", s); // read line with <contributor>XXX...
          f = 3;
        }
      }
    } else {
...
int prepr3(char const *argv[]) {
  ...
  sf = fopen(argv[0], "rb");
  if (sf == 0) {
//    printf("can't open 1st file"), exit(1);
  }
  ...
  t2 = fopen(argv[1], "wb");
  if (t2 == 0) {
//    printf("can't open 2nd file"), exit(1);
  }
...
//        printf("Assign c = 1, f = %d, %s\n", f, tmp);
      }
    }
    for (i = 0; i < j; i++) {
      if (*(int *)&p1[i] == 'et/<') { //detected <\text> in tmp1a
        c = 0;
//        printf("Assign c = 0, f = %d, %s\n", f, tmp);
      }
    }
...
int resto2(char const* argv[])
{
  ...
  sf = fopen(argv[0],"rb");
//  if (sf==0)  printf("can't open 1st file"), exit(1);
...
//    printf("diff=%ld\n", p4 - p3);
  ...
  t2 = fopen(argv[1],"wb");
//  if (t2==0)  printf("can't open 2nd file"), exit(1);
...
int resto3(char const* argv[]) {
  ...
  unsigned char *p1 = (unsigned char *)malloc(R3_INPUT_SIZE),
                *p0 = p1,
//                *p3 = (unsigned char *)calloc(OUTPUT_ARR_SIZE, sizeof(char)),
